...
@section('content')
		...
		<div class="col-md-12 "> 
						...
						<tr>
				            ...
				            <td class="status_{{$value->id}}">
				            	@if($value->deliver_status == 0)
				                    <button type="button" class="btn btn-success">{{"Chờ xử lý"}}</button>
				                @elseif($value->deliver_status == 1)
				                    <button type="button" class="btn btn-danger">{{"Đang giao hàng"}}</button>
				                @else 
				                    <button type="button" class="btn btn-dark">{{"Hoàn thành"}}</button>
				                @endif
				            </td>
			...
			<div style="float: left;" class="col-md-4 ">
				...
				<table class="table table-striped table-bordered table-hover">
					<tbody>
						@foreach($list_selling as $value)
						<tr>
							<td>
								@foreach($value->product_images as $key=>$valus)
				                @if($value->id == $valus->product_id)
				                @if($key == 0)
				                	<img src="/uploads/{{$valus->path}}" alt="" width="70px"></img>
				                @endif
				                @endif
				                @endforeach
							</td>
				            <td>
				            	<p>Lượt mua: {!!$value->sales_volume!!}  </p><br>
				            	{{$value->name}}
				            </td>
				            <td>${{$value->price}}</td>
...
<!-- 				<h4>Top sản phẩm được yêu thích</h4>
				<table class="table table-striped table-bordered table-hover">
					<thead>
						<tr>
							<th scope="col">STT</th>
